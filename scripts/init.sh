...
ensure_pkg_manager() {
    if [ "${PKG_MANAGER}" = "none" ]; then
        log_error "未检测到 dnf/yum，无法在 openEuler 上安装系统依赖"
        exit 1
    fi
}
...
setup_repositories() {
    show_step "配置软件源 (openEuler DNF/YUM)"

    if [ "${SKIP_OPENEULER_REPO_MIRROR:-}" = "1" ]; then
        log_info "已设置 SKIP_OPENEULER_REPO_MIRROR=1，跳过软件源修改"
        pkg_makecache
        return 0
    fi

    local mirror="${OPENEULER_REPO_MIRROR:-https://mirrors.aliyun.com/openeuler}"
    ...
    for repo_file in "$repo_dir"/*.repo; do
        ...
        if grep -Eq 'https?://repo\.openeuler\.(org|openatom\.cn)' "$repo_file" 2>/dev/null; then
            sed -i \
                -e "s#http://repo.openeuler.org#${mirror}#g" \
                -e "s#https://repo.openeuler.org#${mirror}#g" \
                -e "s#http://repo.openeuler.openatom.cn#${mirror}#g" \
                -e "s#https://repo.openeuler.openatom.cn#${mirror}#g" \
                "$repo_file" || true
            changed=true
        fi
    done
    ...
    if [ "$changed" = true ]; then
        ...
        log_info "未检测到 repo.openeuler.*，保持当前软件源不变"
    fi
}
